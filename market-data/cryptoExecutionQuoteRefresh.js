...
export function createCryptoExecutionQuoteRefresher({
  getLatestQuotes,
  normalizeSymbol,
  updateQuoteCache,
  getCachedQuote = () => null,
  ...
  return async function refreshCryptoExecutionQuotes(signals = []) {
    ...
    return sourceSignals.map((signal) => {
      ...
      const cached = (quote ? updateQuoteCache(symbol, {
        ...
        liveQuoteSource:
          quote.liveQuoteSource || quote.source || "alpaca_crypto_latest",
      ...
      const refreshed = {
        ...
        cryptoRealism: { ...(signal.cryptoRealism || {}), spreadAvailable: cached.spreadAvailable === true, spreadPercent: cached.spreadPercent },
        ...
        liveQuoteSource:
          cached.liveQuoteSource || cached.source || "alpaca_crypto_latest",
