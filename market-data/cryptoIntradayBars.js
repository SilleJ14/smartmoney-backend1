...
export function createCryptoIntradayBars({ getRecentBars, normalizeSymbol, maxSymbols = 250, now = Date.now }) {
  const cache = new Map();
  const pending = new Map();
  const capacity = Math.max(1, Math.min(300, Number(maxSymbols) || 250));
  function retain(symbol, bars, ttl) {
    cache.delete(symbol);
    cache.set(symbol, { bars: bars.slice(-30), expiresAt: now() + ttl });
