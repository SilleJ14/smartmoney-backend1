...
export function evaluateTrendContinuation({ h1 = [], m15 = [], h4 = [], side = "buy" } = {}) {
  ...
  const support = side === "buy" ? swings.lastLow?.price : swings.lastHigh?.price;
  const A = atrBeforeWindow(hours, hours.length, 14) || atrBeforeWindow(hours, hours.length - 1, 14);
  ...
  const frozen = { A, support, reference, side };
  const priorTwoHigh = Math.max(num(after[after.length - 2]?.h), num(after[after.length - 3]?.h));
  const priorTwoLow = Math.min(num(after[after.length - 2]?.l), num(after[after.length - 3]?.l));
  ...
  const sequence = [...pre.slice(-4), ...after];
  const stop = side === "buy"
