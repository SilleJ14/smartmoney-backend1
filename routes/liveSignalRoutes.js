...
export function registerLiveSignalRoutes(app, dependencies) {
  const { requireAdmin, getState, getConfig = () => ({}), runFastRunnerEngine, getTopSignals, mergeLiveQuote,
  ...
  app.get("/fast-runners", requireAdmin, async (req, res) => {
    try {
      ...
      const candidates = (current.fastRunnerCandidates || []).map(mergeLiveQuote).filter(signal => candidateFeedDecision(signal, getConfig()).visible);
      res.json({ ok: true, generatedAt: now().toISOString(), state: current.fastRunnerEngineState || null,
        count: candidates.length, candidates,
        visibleCandidates: (current.visibleLiveCandidates || []).map(mergeLiveQuote).filter(signal => candidateFeedDecision(signal, getConfig()).visible) });
  ...
  app.get("/live-signals", requireAdmin, async (_req, res) => {
    try {
      ...
      const stockSignals = getTopSignals([...(state.lastStockSignals || []), ...(state.fastRunnerCandidates || []),
        ...(state.quickInstitutionalCandidates || [])].map(mergeLiveQuote).filter(signal => candidateFeedDecision(signal, getConfig()).visible), 25);
